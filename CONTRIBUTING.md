...
The main objects that users will interact with are the `cohesivenet.VNS3Client` and the `cohesivenet.MSClient` for interacting with the VNS3 API and MS API respectively.

The API calls are located in `cohesivenet.api.vns3` and `cohesivenet.api.vns3ms` and grouped into named modules. These are reflected in the API specifications by the `x-sdk-module` property defined on the method specifications.
