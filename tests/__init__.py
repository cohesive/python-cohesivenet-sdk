...
def assert_types(response_dict, types_dict):
    ...
    """
    required_keys = set(
        [k for k, type_info in types_dict.items() if type_info.required]
    )
    response_keys = set(response_dict.keys())
    missing_keys = required_keys.difference(response_keys)
    assert not missing_keys, "Missing required keys: %s" % ",".join(
    ...
    for key, val in response_dict.items():
        ...
        if not isinstance(val, val_type_info.type):
            type_errors.append(
                'Invalid type for "%s". Must be "%s" but was "%s"'
                % (key, val_type_info.type, type(val))
            )
