...
def fetch_state_attribute(clients, attribute, bust_cache=False) -> data_types.BulkOperationResult:
    ...
    """
    assert attribute in _StateLibrary, 'Attribute %s not currently supported' % attribute
    fetch_func = _StateLibrary.get(attribute)
    api_calls = [
        bind(fetch_func, client, bust_cache=bust_cache)
